...
# instead of all-reducing grad tensors one at a time, after loss.backward()
# it aggregates into 1 long 1-D tensor, and does all-reduce on that.
# Thus lowers communication calls. 
# Improves performance by lowering the overhead associated with issuing a 
# large number of small all-reduce operations!
'''
...
def dist_demo (rank, world_size, train_dataset, num_epochs, worker_batch_size, device):
    ...
    try:
        ...
        #Verify flattened_ddp in process 0!        
        if rank == 0:
